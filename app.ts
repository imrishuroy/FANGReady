...
class DSATracker {
    ...
    private sortBy: 'default' | 'difficulty' | 'frequency' = 'default';
    ...
    private toggleQuestion(id: string): void {
    ...
    private updateQuestionRow(id: string): void {
    ...
    private getFilteredQuestions(): Question[] {
        let filtered = questions.filter(q => {
            ...
            if (this.filters.company && !q.companies.includes(this.filters.company)) return false;
            if (this.filters.frequency && q.frequency !== this.filters.frequency) return false;
            if (this.filters.status === 'completed' && !this.completedQuestions.has(q.id)) return false;
            if (this.filters.status === 'pending' && this.completedQuestions.has(q.id)) return false;
            return true;
        });
    ...
    private populateFilters(): void {
        const categories: string[] = [...new Set(questions.map(q => q.category))].sort();
        const companies: string[] = [...new Set(questions.flatMap((q: Question) => q.companies))] as string[];
        companies.sort();
    ...
    private setupEventListeners(): void {
        ...
        document.getElementById('difficultyFilter')!.addEventListener('change', (e) => {
            this.filters.difficulty = (e.target as HTMLSelectElement).value;
            this.render();
        });

        document.getElementById('companyFilter')!.addEventListener('change', (e) => {
            this.filters.company = (e.target as HTMLSelectElement).value;
            this.render();
        });
        ...
        document.getElementById('clearFilters')!.addEventListener('click', () => {
            ...
            (document.getElementById('frequencyFilter') as HTMLSelectElement).value = '';
        ...
        if (filtered.length === 0) {
            ...
            container.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
                checkbox.addEventListener('change', (e) => {
                    e.stopPropagation();
                    const id = (e.target as HTMLInputElement).dataset.questionId!;
