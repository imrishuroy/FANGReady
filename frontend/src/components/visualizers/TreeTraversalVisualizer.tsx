...
export default function TreeTraversalVisualizer() {
  ...
  const [message, setMessage] = useState('Select traversal type and click Play');

  const getTraversalSteps = useCallback((type: TraversalType): { node: number; action: 'visit' | 'process' }[] => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (phase === 'init') {
        ...
        const orderDesc = traversalType === 'preorder' ? 'Root → Left → Right'
  ...
  }, [isPlaying, phase, stepIndex, traversalType, result, getTraversalSteps, speed]);
      ...
      <div className="p-4">
        ...
        <div className="mb-4 p-3 bg-gray-800/50 rounded-lg">
          <div className="text-sm text-gray-400 mb-1">
            {traversalType === 'preorder' && 'Preorder: Process node BEFORE children (Root → Left → Right)'}
            {traversalType === 'inorder' && 'Inorder: Process node BETWEEN children (Left → Root → Right)'}
            {traversalType === 'postorder' && 'Postorder: Process node AFTER children (Left → Right → Root)'}
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 min-h-[40px] items-center">
            ...
            {result.length === 0 && <span className="text-gray-500 italic">Empty</span>}
          </div>
        </div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p className="mb-1"><strong className="text-teal-400">Expected:</strong></p>
          <p>Preorder: [1, 2, 4, 5, 3] | Inorder: [4, 2, 5, 1, 3] | Postorder: [4, 5, 2, 3, 1]</p>
        </div>
      </div>
    </div>
  );
