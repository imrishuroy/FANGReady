...
export default function MergeKListsVisualizer() {
  ...
  const [phase, setPhase] = useState<'init' | 'initializing' | 'extracting' | 'adding' | 'done'>('init');
  const [currentExtracted, setCurrentExtracted] = useState<HeapNode | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (phase === 'init') {
        ...
        setPointers(initialLists.map(() => 0));
        setPhase('extracting');
        setMessage(`Heap initialized with first elements: [${newHeap.map(n => n.value).join(', ')}]`);
        ...
        if (currentExtracted) {
          ...
          if (nextIndex < initialLists[listIndex].length) {
            ...
            const newHeap = [...heap, { value: nextValue, listIndex, nodeIndex: nextIndex }];
            setHeap(sortHeap(newHeap));
            setMessage(`Add next from list ${listIndex + 1}: ${nextValue}. Heap: [${sortHeap(newHeap).map(n => n.value).join(', ')}]`);
          } else {
            setMessage(`List ${listIndex + 1} exhausted. No more elements to add.`);
          }
        }
        setCurrentExtracted(null);
        setPhase('extracting');
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-violet-500/10 to-purple-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Merge K Sorted Lists</h3>
      ...
      <div className="p-4">
        ...
        {/* K Sorted Lists */}
        <div className="mb-6">
          <div className="text-sm text-gray-400 mb-2">K = {initialLists.length} Sorted Lists:</div>
          <div className="space-y-2">
            {initialLists.map((list, listIdx) => (
              <div key={listIdx} className="flex items-center gap-2">
                <span className={`w-6 h-6 rounded ${LIST_COLORS[listIdx]} flex items-center justify-center text-white text-xs font-bold`}>
                  {listIdx + 1}
                </span>
                <div className="flex gap-1">
                  ...
                  {pointers[listIdx] >= list.length && (
                    <span className="text-gray-500 text-sm ml-2 self-center">exhausted</span>
                  )}
                </div>
              </div>
            ))}
        ...
        {/* Min-Heap */}
        <div className="mb-6">
          <div className="text-sm text-gray-400 mb-2">Min-Heap (current front pointers):</div>
              ...
              <AnimatePresence>
                {heap.length > 0 ? (
                  ...
                  >
                    ...
                    >
                      {heap[0].value}
                      <span className="absolute -top-5 text-xs text-violet-400">min</span>
                    </motion.div>
                    {/* Children */}
                    {heap.length > 1 && (
        ...
        {currentExtracted && (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className={`px-3 py-1 rounded ${LIST_COLORS[currentExtracted.listIndex]} text-white font-bold`}>
                {currentExtracted.value}
              </span>
              <span className="text-gray-400 text-sm">from List {currentExtracted.listIndex + 1}</span>
            </div>
          </motion.div>
        )}
        ...
        <div className="mb-4">
          ...
          <div className="bg-gray-800/50 rounded-lg p-3 min-h-[50px]">
            <div className="flex gap-1 flex-wrap">
              ...
              {result.length === 0 && (
                <span className="text-gray-500">Elements will appear here...</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-violet-400">{heap.length}</div>
            <div className="text-xs text-gray-500">Heap Size</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-green-400">{result.length}</div>
            <div className="text-xs text-gray-500">Merged Count</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-blue-400">
              {initialLists.reduce((sum, l) => sum + l.length, 0) - result.length}
            </div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p>
            ...
            Keep a min-heap of size K (one element from each list). Extract min, add to result,
            then push the next element from that list. Time: O(N log K) where N is total elements.
          </p>
        </div>
      </div>
    </div>
