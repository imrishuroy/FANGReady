...
export default function KthLargestVisualizer() {
  ...
  const [message, setMessage] = useState('Click Play to find the 3rd largest element');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (newHeap.length < k) {
        ...
        setHeap(newHeap);
        setLastAction('add');
        setMessage(`Added ${num} to heap. Heap size ${newHeap.length} < k=${k}, so we keep it.`);
        ...
        setHeap(newHeap);
        setLastAction('add');
        setMessage(`${num} > heap min (${removed}). Removed ${removed}, added ${num}. Heap now: [${newHeap.join(', ')}]`);
      } else {
        setLastAction(null);
        setMessage(`${num} <= heap min (${newHeap[0]}). Skip - not in top ${k}.`);
      }
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Kth Largest Element</h3>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="text-gray-400 text-sm">
            Finding the <span className="text-amber-400 font-bold">{k}rd largest</span> element
          </div>
        </div>
        ...
        {/* Array Visualization */}
        <div className="mb-6">
          <div className="text-sm text-gray-400 mb-2">Input array: [{nums.join(', ')}]</div>
        ...
        <div className="mb-6">
          ...
          <div className="bg-gray-800/50 rounded-lg p-4 min-h-[100px]">
            ...
            <div className="flex flex-col items-center gap-2">
              <AnimatePresence>
                {heap.length > 0 && (
                  ...
                  >
                    ...
                    >
                      {heap[0]}
                      <span className="absolute -top-5 text-xs text-amber-400">min (root)</span>
                    </motion.div>
              ...
              {heap.length === 0 && (
                <span className="text-gray-500">Empty - will store top {k} largest</span>
              )}
            </div>
                ...
                {heap.map((num, i) => (
                  <span
                    ...
                    className={`px-2 py-1 rounded text-sm font-mono ${
                      i === 0 ? 'bg-amber-500 text-black' : 'bg-gray-700 text-gray-300'
                    }`}
                  >
                    {num}
                  </span>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-amber-400">{heap.length}</div>
            <div className="text-xs text-gray-500">Heap Size</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-green-400">{heap[0] ?? '-'}</div>
            <div className="text-xs text-gray-500">Heap Min (Kth Largest)</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-blue-400">{currentIndex}/{nums.length}</div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p>
            ...
            If a new number is larger than the heap&apos;s min, swap them.
          </p>
        </div>
      </div>
    </div>
