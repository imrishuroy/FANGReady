...
export default function PermutationsVisualizer() {
  ...
  const [message, setMessage] = useState('Click Play to generate all permutations');
  const [stepIndex, setStepIndex] = useState(-1);
  const [steps, setSteps] = useState<{ path: number[]; used: boolean[]; action: 'add' | 'choose' | 'backtrack'; index: number }[]>([]);

  const generateSteps = useCallback(() => {
    const allSteps: { path: number[]; used: boolean[]; action: 'add' | 'choose' | 'backtrack'; index: number }[] = [];

    function backtrack(path: number[], usedArr: boolean[]) {
      if (path.length === nums.length) {
        allSteps.push({ path: [...path], used: [...usedArr], action: 'add', index: -1 });
        return;
      }
      ...
      for (let i = 0; i < nums.length; i++) {
        ...
        usedArr[i] = true;
        path.push(nums[i]);
        allSteps.push({ path: [...path], used: [...usedArr], action: 'choose', index: i });
        ...
        if (path.length < nums.length - 1 || i < nums.length - 1) {
          allSteps.push({ path: [...path], used: [...usedArr], action: 'backtrack', index: i });
        }
      }
    }

    allSteps.push({ path: [], used: [false, false, false], action: 'choose', index: -1 });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (nextStepIdx >= steps.length) {
        setPhase('done');
        setMessage(`Done! Generated all ${results.length} permutations (${nums.length}! = ${results.length})`);
        setIsPlaying(false);
        return;
      }
      ...
      if (step.action === 'add') {
        ...
        setMessage(`Complete permutation! Add [${step.path.join(', ')}] to results`);
      } else if (step.action === 'choose') {
        if (step.index === -1) {
          setMessage('Start exploring permutations');
        } else {
          setMessage(`Choose ${nums[step.index]}, mark as used, path = [${step.path.join(', ')}]`);
        }
      } else {
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Permutations Generator</h3>
      ...
      <div className="p-4">
          ...
          <div className="flex gap-2 justify-center">
            {nums.map((num, idx) => (
              ...
              >
                ...
                <span className={`text-xs ${used[idx] ? 'text-red-300' : 'text-green-300'}`}>
                  {used[idx] ? 'used' : 'free'}
                </span>
              </motion.div>
            ))}
        ...
        <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
          <div className="text-sm text-gray-400 mb-1">Current Permutation (building):</div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p>
            <strong className="text-blue-400">Key Difference from Subsets:</strong>{' '}
            We use a <code className="text-cyan-400">used[]</code> array instead of a start index.
            This lets us pick any unused element at each position.
          </p>
        </div>
      </div>
    </div>
