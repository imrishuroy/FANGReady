...
export default function AnagramGroupVisualizer() {
  ...
  const [phase, setPhase] = useState<'init' | 'sorting' | 'grouping' | 'done'>('init');
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      ...
      <div className="p-4">
          ...
          >
            <div className="flex items-center gap-4 flex-wrap">
              <div>
                ...
                <div className="text-2xl font-mono font-bold text-yellow-400">{currentWord}</div>
              </div>
              <div className="text-2xl text-gray-500">→</div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Sort Characters</div>
              ...
              {currentKey && (
                <>
                  <div className="text-2xl text-gray-500">→</div>
                  <div>
                    <div className="text-xs text-gray-500 mb-1">Key (sorted)</div>
                    <div className="text-2xl font-mono font-bold text-green-400">&quot;{currentKey}&quot;</div>
                  </div>
                </>
              )}
            </div>
        ...
        {/* Groups HashMap */}
        <div className="mb-4">
          <div className="text-sm text-gray-400 mb-2">HashMap: sorted_key → [words]</div>
          <div className="bg-gray-800/50 rounded-lg p-4 min-h-[100px]">
            <div className="space-y-2">
              <AnimatePresence>
                {Array.from(groups.entries()).map(([key, wordList], groupIdx) => (
                  <motion.div
                    key={key}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`px-2 py-1 rounded font-mono text-sm ${GROUP_COLORS[groupIdx % GROUP_COLORS.length]} text-white`}>
                        &quot;{key}&quot;
                      </span>
              ...
              {groups.size === 0 && (
                <span className="text-gray-500">Empty - groups will appear here</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-indigo-400">{words.length}</div>
            <div className="text-xs text-gray-500">Total Words</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-green-400">{currentIndex}</div>
            <div className="text-xs text-gray-500">Processed</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-purple-400">{groups.size}</div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p>
            ...
            Anagrams have the same characters, so sorting them produces the same key.
            Use this sorted string as the HashMap key to group all anagrams together.
          </p>
        </div>
      </div>
    </div>
