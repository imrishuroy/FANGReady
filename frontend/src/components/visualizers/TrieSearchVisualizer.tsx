...
export default function TrieSearchVisualizer() {
    ...
    const timer = setTimeout(() => {
      if (phase === 'init') {
        ...
        if (!node?.children[char]) {
          // Character not found
          setResult(false);
          setMessage(`'${char}' not found in Trie. ${searchMode}("${query}") = false`);
          setPhase('done');
          setIsPlaying(false);
          return;
        }
  ...
  }, [isPlaying, phase, charIndex, query, currentPath, searchMode, trie, speed]);
  ...
  const renderTrieNode = (node: TrieNode, path: string[], char: string) => {
    ...
    const isInPath = currentPathStr.startsWith(pathStr) && pathStr.length <= currentPathStr.length;
    ...
    return (
      <g key={pathStr || 'root'}>
        <motion.circle
          ...
          r={18}
          animate={{
            fill: isCurrent ? '#eab308' : isInPath ? '#3b82f6' : node.isEnd ? '#22c55e' : '#374151',
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-indigo-500/10 to-blue-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Search vs StartsWith</h3>
      ...
      <div className="p-4">
        ...
        <div className="mb-4 p-3 bg-gray-800/50 rounded-lg">
          <div className="text-sm text-gray-400 mb-2">Select query (Trie contains: {words.join(', ')}):</div>
        ...
        <div className="mb-4 flex justify-center">
          <svg width="400" height="360" className="bg-gray-800/30 rounded-lg">
            ...
            {renderTrieNode(trie.children.a.children.p.children.p, ['a', 'p', 'p'], 'p')}
            {renderTrieNode(trie.children.a.children.p.children.e, ['a', 'p', 'e'], 'e')}
            {renderTrieNode(trie.children.b.children.a.children.t, ['b', 'a', 't'], 't')}
            {renderTrieNode(trie.children.a.children.p.children.p.children.l, ['a', 'p', 'p', 'l'], 'l')}
            {renderTrieNode(trie.children.a.children.p.children.p.children.l.children.e, ['a', 'p', 'p', 'l', 'e'], 'e')}
          </svg>
        </div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p>
            ...
            search("appl") = false (not a word), but startsWith("appl") = true (prefix exists).
            * marks isEnd = true.
          </p>
        </div>
      </div>
