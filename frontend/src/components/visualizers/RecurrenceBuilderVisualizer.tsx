    ...
    steps: [
      {
        ...
        explanation: 'At each step, I must decide: take 1 step or take 2 steps?'
      },
      {
        question: 'What are my choices?',
        answer: 'Take 1 step OR Take 2 steps',
        explanation: 'These are the only two options available at any position.'
      },
      {
        question: 'What happens after each choice?',
        answer: 'Smaller subproblem remains',
        explanation: '• Take 1 step → solve for (n-1) remaining\n• Take 2 steps → solve for (n-2) remaining'
      },
      {
  ...
  'house-robber': {
    ...
    steps: [
      ...
      },
      {
        question: 'What are my choices?',
        answer: 'ROB or SKIP',
        explanation: '• ROB: Take money, must skip next house\n• SKIP: Move to next house'
      },
      {
        question: 'What happens after each choice?',
        answer: 'Different subproblems',
        explanation: '• ROB: Get nums[i] + solve from (i+2)\n• SKIP: Solve from (i+1)'
      },
      {
        question: 'How do I combine results?',
  ...
  'coin-change': {
    ...
    steps: [
      ...
      },
      {
        ...
        explanation: '• Use coin c → solve for (amount - c)\n• Each coin can be used multiple times'
      },
      {
        question: 'How do I combine results?',
  ...
  'lcs': {
    ...
    steps: [
      ...
      },
      {
        ...
        explanation: '• If match: take both characters\n• If no match: skip from s1 OR skip from s2'
      },
      {
        question: 'What happens after each choice?',
        answer: 'Move indices accordingly',
        explanation: '• Match: both indices move (i-1, j-1)\n• Skip: one index moves'
      },
      {
        question: 'How do I combine results?',
        answer: 'Match: +1, No match: MAX',
        explanation: '• Match: 1 + LCS(i-1, j-1)\n• No match: max(LCS(i-1, j), LCS(i, j-1))'
      }
    ],
    recurrence: 'lcs(i,j) = match ? 1+lcs(i-1,j-1) : max(lcs(i-1,j), lcs(i,j-1))',
    baseCase: 'lcs(0,j) = 0, lcs(i,0) = 0'
  }
};

export default function RecurrenceBuilderVisualizer() {
  const [selectedProblem, setSelectedProblem] = useState<Problem>('climbing-stairs');
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-indigo-500/10 to-violet-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Recurrence Relation Builder</h3>
        <p className="text-gray-400 text-sm mt-1">Learn to derive recurrence relations step-by-step</p>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="mb-6">
          <h4 className="text-white font-medium mb-3 flex items-center gap-2">
            <span className="w-6 h-6 bg-indigo-500 rounded-full flex items-center justify-center text-xs">?</span>
                ...
                <div className="p-4">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`font-medium ${
                      revealedSteps.has(idx) ? 'text-indigo-300' : 'text-gray-400'
                    }`}>
                      {step.question}
                    </span>
                  </div>
                      ...
                      >
                        <div className="ml-11 space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white font-semibold">{step.answer}</span>
                          </div>
          ...
          {showRecurrence && (
            ...
            >
              ...
              <div className="text-center">
                ...
                <code className="text-yellow-400 font-mono">{problem.baseCase}</code>
              </div>

              <div className="mt-4 p-3 bg-gray-800/50 rounded-lg">
                <p className="text-gray-300 text-sm text-center">
                  Now you can implement this as recursion → memoization → tabulation → space optimized!
                </p>
              </div>
            </motion.div>
          )}
        ...
        <div className="mt-6 p-4 bg-gray-800/30 rounded-xl">
          <h5 className="text-gray-400 text-sm font-medium mb-2">Combination Patterns:</h5>
