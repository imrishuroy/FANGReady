...
export default function ActivitySelectionVisualizer() {
  ...
  const [phase, setPhase] = useState<'unsorted' | 'sorting' | 'sorted' | 'selecting'>('unsorted');
  ...
  const sortedActivities = [...originalActivities].sort((a, b) => a.end - b.end);

  const [activities, setActivities] = useState(originalActivities);
  const [message, setMessage] = useState('Activities shown in original order. Click Play to start!');
    ...
    const timer = setTimeout(() => {
      if (phase === 'unsorted') {
        ...
        if (currentIdx >= sortedActs.length) {
          setIsPlaying(false);
          setMessage(`Done! Selected ${selectedActivities.size} non-overlapping activities.`);
          return;
        }
        ...
        if (act.start >= lastEnd) {
          ...
          setLastEnd(act.end);
          setMessage(`Selected ${act.name} (ends at ${act.end}). Last end = ${act.end}`);
        } else {
          setMessage(`Skipped ${act.name} (starts at ${act.start} < lastEnd ${lastEnd})`);
        }
  ...
  }, [isPlaying, phase, currentActivity, lastEnd, selectedActivities.size, speed, sortedActivities]);
  ...
  const getActivityStyle = (act: Activity) => {
    ...
    const isCurrent = phase === 'selecting' && sortedActivities[currentActivity ?? -1]?.id === act.id;
    const isSkipped = phase === 'selecting' &&
      currentActivity !== null &&
      sortedActivities.slice(0, currentActivity).some(a => a.id === act.id) &&
      !selectedActivities.has(act.id);

    if (isCurrent) return 'ring-4 ring-yellow-400 ring-offset-2 ring-offset-gray-900';
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Activity Selection Visualizer</h3>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 mb-4">
          <button
            ...
            disabled={phase === 'selecting' && (currentActivity ?? 0) >= sortedActivities.length}
        ...
        <div className="flex gap-2 mb-4">
          {['unsorted', 'sorting', 'sorted', 'selecting'].map((p, i) => (
            <div
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${
                phase === p
                  ...
                  : i < ['unsorted', 'sorting', 'sorted', 'selecting'].indexOf(phase)
            ...
            >
              ...
              {p === 'unsorted' ? 'Original' : p === 'sorting' ? 'Sort' : p === 'sorted' ? 'Ready' : 'Select'}
            </div>
          ))}
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="relative h-64 bg-gray-800/50 rounded-lg overflow-hidden">
            ...
            {phase === 'selecting' && lastEnd > 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1, left: `${(lastEnd / timelineMax) * 100}%` }}
        ...
        <motion.div
          ...
          className={`p-3 rounded-lg mb-4 ${
            message.includes('Selected') ? 'bg-green-500/10 border border-green-500/30 text-green-400' :
            message.includes('Skipped') ? 'bg-red-500/10 border border-red-500/30 text-red-400' :
            message.includes('Sorting') ? 'bg-yellow-500/10 border border-yellow-500/30 text-yellow-400' :
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-green-400">{selectedActivities.size}</div>
            <div className="text-xs text-gray-500">Selected</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-gray-400">
              {phase === 'selecting' ? currentActivity ?? 0 : 0}/{activities.length}
            </div>
