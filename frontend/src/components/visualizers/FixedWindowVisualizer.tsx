...
export default function FixedWindowVisualizer() {
  ...
  const [phase, setPhase] = useState<'init' | 'building' | 'sliding' | 'done'>('init');
  const [message, setMessage] = useState('Click Play to find maximum sum of k=3 consecutive elements');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (phase === 'init') {
        ...
        setWindowSum(newSum);
        setMessage(`Adding arr[${currentIdx}] = ${arr[currentIdx]} to window. Sum = ${newSum}`);
        setCurrentIdx(currentIdx + 1);
      } else if (phase === 'sliding') {
        if (currentIdx >= arr.length) {
          setPhase('done');
          setMessage(`Done! Maximum sum = ${maxSum} at window [${maxWindow![0]}, ${maxWindow![1]}]`);
          setIsPlaying(false);
          return;
        }
        ...
        setWindowSum(newSum);
        setMessage(`Remove arr[${currentIdx - k}] = ${leftVal}, Add arr[${currentIdx}] = ${rightVal}. Sum = ${newSum}`);
  ...
  }, [isPlaying, phase, currentIdx, windowSum, maxSum, maxWindow, arr, k, speed]);
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Fixed Size Sliding Window</h3>
      ...
      <div className="p-4">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            {arr.map((val, idx) => {
              ...
              const isMaxWindow = maxWindow && idx >= maxWindow[0] && idx <= maxWindow[1];
              ...
              {arr.map((_, idx) => (
                <div
                  ...
                  className={`w-14 h-1 rounded ${
                    idx >= winLeft && idx <= winRight ? 'bg-blue-500' : 'bg-transparent'
                  }`}
                />
              ))}
            </div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p>
            ...
            Instead of summing k elements each time O(k), update incrementally:
            newSum = oldSum + arr[right] - arr[left]. O(1) per slide!
          </p>
        </div>
      </div>
    </div>
