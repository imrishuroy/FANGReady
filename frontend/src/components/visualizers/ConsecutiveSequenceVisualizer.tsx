...
interface NumberState {
  value: number;
  state: 'unseen' | 'inSet' | 'checking' | 'sequenceStart' | 'inSequence' | 'notStart';
...
export default function ConsecutiveSequenceVisualizer() {
  ...
  const [phase, setPhase] = useState<'init' | 'building-set' | 'scanning' | 'extending' | 'done'>('init');
  const [scanIndex, setScanIndex] = useState(0);
  const [sequenceCount, setSequenceCount] = useState(0);
  const [message, setMessage] = useState('Click Play to find longest consecutive sequence');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (phase === 'init') {
        ...
        setNumSet(newSet);
        const newNumbers = numbers.map(n => ({ ...n, state: 'inSet' as const }));
        setNumbers(newNumbers);
        setPhase('scanning');
        setMessage('Step 2: Scan each number, only start counting from sequence beginnings');
      } else if (phase === 'scanning') {
        if (scanIndex >= numbers.length) {
          setPhase('done');
          setMessage(`Done! Longest consecutive sequence: [${longestSequence.join(', ')}] with length ${longestSequence.length}`);
          setIsPlaying(false);
          return;
        }
        ...
        if (numSet.has(num - 1)) {
          setMessage(`${num}: Has predecessor (${num - 1} exists), skip - not a sequence start`);
          setTimeout(() => {
          ...
          setMessage(`${num}: No predecessor (${num - 1} not in set) - this is a sequence START!`);
          setPhase('extending');
          setCurrentSequence([num]);
          setSequenceCount(sequenceCount + 1);

          const updated = [...newNumbers];
          updated[idx] = { ...updated[idx], state: 'sequenceStart', sequenceId: sequenceCount };
          setNumbers(updated);
        }
        ...
        if (numSet.has(nextNum)) {
          ...
          setMessage(`Extending: ${lastNum} + 1 = ${nextNum} exists in set! Sequence: [${newSequence.join(', ')}]`);

          const newNumbers = [...numbers];
          const idx = newNumbers.findIndex(n => n.value === nextNum);
          if (idx !== -1) {
            newNumbers[idx] = { ...newNumbers[idx], state: 'inSequence', sequenceId: sequenceCount };
            setNumbers(newNumbers);
          }
        } else {
          setMessage(`${nextNum} not in set. Sequence complete: [${currentSequence.join(', ')}] length = ${currentSequence.length}`);
  ...
  }, [isPlaying, phase, scanIndex, numbers, numSet, currentSequence, longestSequence, sequenceCount, speed]);
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-teal-500/10 to-cyan-500/10 border-b border-gray-800">
        <h3 className="text-lg font-semibold text-white">Longest Consecutive Sequence</h3>
      ...
      <div className="p-4">
        ...
        {/* Original Array */}
        <div className="mb-4">
          <div className="text-sm text-gray-400 mb-2">Original array (unsorted):</div>
        ...
        <div className="mb-4">
          <div className="text-sm text-gray-400 mb-2">Sorted view (for visualization only):</div>
        ...
        {currentSequence.length > 0 && (
          ...
          >
            ...
            <div className="flex gap-2 items-center">
              ...
              <span className="ml-2 text-teal-400 font-bold">Length: {currentSequence.length}</span>
            </div>
          </motion.div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-cyan-400">{longestSequence.length}</div>
            <div className="text-xs text-gray-500">Longest Found</div>
            {longestSequence.length > 0 && (
              <div className="text-xs text-gray-400 mt-1">[{longestSequence.join(', ')}]</div>
            )}
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-purple-400">{sequenceCount}</div>
        ...
        <div className="mt-4 p-3 bg-gray-800/30 rounded-lg text-sm text-gray-400">
          <p>
            <strong className="text-teal-400">Key Insight:</strong>{' '}
            Only start counting from sequence beginnings (numbers where n-1 doesn&apos;t exist).
            Each number is visited at most twice: once in scan, once when extending a sequence. O(n) total.
          </p>
        </div>
      </div>
    </div>
